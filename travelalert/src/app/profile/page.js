...
function planLabel(plan) {
  ...
  if (plan === "trip_pass") return "Trip Pass (30 days)";
  return "Explorer Plan (3 free cities)";
}
...
function ProfileContent() {
  ...
  const membershipSubtitle =
    plan === "annual"
      ...
      : plan === "trip_pass"
        ? "Trip Pass · Active"
        : "Explorer Plan · Click to upgrade";
    ...
    <main className="min-h-screen bg-[#f7f6f2] px-4 py-6 text-zinc-900 transition-colors duration-200 sm:py-10 dark:bg-[#0c0c0e] dark:text-[#f3f3f2]">
      <div className="mx-auto w-full max-w-[580px] space-y-4 sm:space-y-5">
        ...
        <div className="rounded-[28px] border border-zinc-200/90 bg-white p-6 text-center shadow-sm sm:p-8 dark:border-white/10 dark:bg-[#16161b]">
          ...
          {/* Plan actions — two siblings, no nesting */}
          <div className="mt-5 flex flex-wrap items-center justify-center gap-2">
            ...
            {plan === "annual" && (
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-full bg-black px-5 py-2.5 text-xs font-bold text-white shadow-sm transition-opacity hover:opacity-90 disabled:opacity-60 dark:border dark:border-white/20 dark:bg-black"
              >
                {billingBusy === "portal" ? (
            ...
            {plan === "annual" &&
              ...
              !subscription?.cancelAtPeriodEnd && (
                <button
                  ...
                  disabled={Boolean(billingBusy)}
                  className="inline-flex items-center gap-1.5 rounded-full border border-zinc-300 bg-white px-4 py-2.5 text-xs font-bold text-zinc-700 transition-colors hover:bg-zinc-100 disabled:opacity-60 dark:border-white/15 dark:bg-[#1c1c24] dark:text-zinc-200 dark:hover:bg-white/10"
                >
                  {billingBusy === "cancel" ? (
                    <LoaderCircle className="size-3.5 animate-spin" />
                  ) : (
