...
export function WeatherCard({ brief, city }) {
  const w = brief?.weather;
  const isRainy = w?.code != null ? RAINY_CODES.includes(w.code) : false;

  // Honest placeholders while the brief loads — never fabricated values.
  const cityName = brief?.city || city || "this destination";
  const temp = w?.temp ?? null;
  const condition = w?.condition ?? null;
  const rainChance = w?.rain_chance ?? null;
  const humidity = w?.humidity ?? null;
  const windSpeed = w?.wind_kph ?? null;
  const sunrise = w?.sunrise ?? null;
  const sunset = w?.sunset ?? null;
  const daylight = w?.daylight ?? null;

  const weeklyForecast = Array.isArray(brief?.forecast) ? brief.forecast : [];
  const dash = "—";
      ...
      <div className="flex h-full w-full flex-col p-3.5 sm:p-4">
        ...
        <div className="mt-2.5 flex items-center justify-center gap-2.5 rounded-full border border-white/5 bg-[#121316] py-2 text-[#f3f3f2] shadow-sm">
          ...
          <span className="font-mono text-sm font-semibold tracking-wide">
            Rain: {rainChance != null ? `${rainChance}%` : dash}
          </span>
        </div>
        ...
        <div className="my-2 flex items-center justify-between px-2 font-mono text-xs font-semibold uppercase tracking-wide text-[#a6a6ad]">
          <span>Humidity: {humidity != null ? `${humidity}%` : dash}</span>
          <span>Wind: {windSpeed != null ? `${windSpeed} Km/h` : dash}</span>
        </div>
              ...
              >
                ...
                <div className="flex items-center justify-center">
                  {item ? (
                    ...
                    <span className="font-mono text-xs text-[#68686f]">·</span>
                  )}
                </div>

                <span className="font-mono text-xs font-semibold text-[#f3f3f2]">
                  {dayTemp != null ? `${dayTemp}°` : dash}
                </span>
              </motion.div>
            );
          })}
